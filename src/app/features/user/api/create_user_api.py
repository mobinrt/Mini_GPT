...
from src.app.features.user.dependencies import get_create_user_use_case

@router.post('/create', 
            response_model=UserDisplay, status_code=status.HTTP_201_CREATED, 
            responses={status.HTTP_400_BAD_REQUEST},
            )
async def create_user(user: UserCreate, create_user_use_case: CreateUserUseCase = Depends(get_create_user_use_case)):
    try:
